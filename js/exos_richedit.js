 ...
 * Model: EXOS_RICHEDIT_V3
 */
(function(global){'use strict';
var API={version:'6.4.0-dev-os84',model:'EXOS_RICHEDIT_V3',ready:true};
...
async function dispatch(ctx,method,args){args=args||[];method=String(method||'');var n,r;
 if(method==='GetVersion')return{version:API.version,model:API.model,backend:'contenteditable host control',compatibility:'RICHEDIT50W_SEMANTIC_V3'};
 ...
 if(method==='ReplaceSelection'||method==='EM_REPLACESEL'){n=ctrl(ctx,args[0]);n.focus();var rep=String(args[1]===undefined?'':args[1]);try{document.execCommand('insertText',false,rep);return true;}catch(e){return false;}}
 ...
 if(method==='SetCharFormat'||method==='EM_SETCHARFORMAT'){n=ctrl(ctx,args[0]);var f=args[1]||{};if(f.bold!==undefined)exec(n,'bold');if(f.italic!==undefined)exec(n,'italic');if(f.underline!==undefined)exec(n,'underline');if(f.strikeout!==undefined)exec(n,'strikethrough');if(f.color)exec(n,'forecolor',f.color);if(f.backgroundColor)exec(n,'hilitecolor',f.backgroundColor);if(f.faceName)exec(n,'fontname',f.faceName);if(f.size)exec(n,'fontsize',Math.max(1,Math.min(7,Math.round(Number(f.size)/3)||3)));return true;}
 ...
 if(method==='StreamIn'||method==='EM_STREAMIN'){n=ctrl(ctx,args[0]);var fmt=String(args[1]||'TEXT').toUpperCase(),content=args[2];if(fmt==='HTML'){n.innerHTML=sanitize(content);return true;}n.textContent=String(content===undefined?'':content);return true;}
