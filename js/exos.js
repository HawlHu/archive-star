 ...
 * Base64 is embedded in exos.js so all CDN nodes use the same image.
 */
...
/* os56: shared image resources moved to exos_share_res.js. */
...
function jplopsoft_taskbarIconName(icon,appId){
  var s=String(icon||''),a=String(appId||'').toLowerCase(),r;
  if(typeof jplopsoft_shareResResolve==='function'){r=jplopsoft_shareResResolve(s,'shell32.dll');if(r)return r.token;}
  if(a==='explorer'||a.indexOf('explorer_')===0)return'res://shell32.dll/explorer';
  if(a==='cmd'||a.indexOf('cmd_')===0)return'res://shell32.dll/cmd';
  if(a==='control')return'res://shell32.dll/control';
...
function jplopsoft_cmdCreateInstance(folderId,parentInstanceId){
  ...
  if(!state.samAuthenticated||!state.vaultKey){
    jplopsoft_exosMessage('請先登入 ExOS，再開啟指令模式。');
    return null;
  }
...
function jplopsoft_cmdWelcome(){
  ...
  jplopsoft_cmdWrite('Empty Enter: commit current command line and show a fresh command line (MS-DOS style)');
  jplopsoft_cmdWrite('');
}
...
function jplopsoft_openFolderInCmd(folderId){
  ...
  if(!state.vaultKey){
    jplopsoft_exosMessage('請先解鎖，再進入 CMD 模式。');
    return;
  }
  ...
  if(id!==0){
    ...
    if(!n||n.type!=='folder'){
      jplopsoft_exosMessage('找不到指定資料夾。');
      return;
    }
...
function jplopsoft_cmdHelp(){
  ...
  jplopsoft_cmdWrite('    .xsh 會以 XSH 沙盒 CreateProcess 啟動；也可直接輸入 program.xsh。');
...
function jplopsoft_cmdSetApplyBuiltin(name,value){
  ...
  if(name==='EXOS_SIDEBAR_WIDTH'){
    ...
    aside=(jplopsoft_explorerCurrentWindow()&&jplopsoft_explorerCurrentWindow().querySelector)?jplopsoft_explorerCurrentWindow().querySelector('.jplopsoft_sidebar'):null;
...
function jplopsoft_openVolume3D(){
  ...
  if(!jplopsoft_threeFeatureAllowed()){
    jplopsoft_exosMessage('Internet Explorer 11 相容模式不提供 3D / Three.js 功能。');
    return;
  }

  if(!state.vaultKey){
    jplopsoft_exosMessage('請先解鎖，再開啟 3D Volume。');
    return;
  }
...
function jplopsoft_cmdStartNode(n){
  ...
  if(fmt==='xsh'){
    parentProc=jplopsoft_cmdActiveProcess();
    jplopsoft_cmdWrite('CreateProcess: starting XSH sandbox '+name+' ...','info');
    jplopsoft_runXshNode(n.id,'',parentProc);
    return;
  }
...
function jplopsoft_cmdExecute(raw){
  ...
  if(jplopsoft_fileExtension(cmd)==='xsh'){
    n=jplopsoft_cmdResolveNode(cmd);
    if(!n){jplopsoft_cmdWrite('The system cannot find the XSH executable.','error');return;}
    jplopsoft_cmdWrite('CreateProcess: starting '+(jplopsoft_decName(n)||cmd)+' ...','info');
    jplopsoft_runXshNode(n.id,arg,jplopsoft_cmdActiveProcess());
    return;
  }
...
function jplopsoft_multiEditorSave(rec){
  ...
  if(plainSize>jplopsoft_TEXT_ONLINE_EDIT_MAX)return jplopsoft_exosMessage('線上編輯內容超過 18 MiB。');
  try{cipher=jplopsoft_encContent(src,jplopsoft_nodeFekById(rec.nodeId));}
  catch(e){return jplopsoft_exosMessage(e.message);}
...
async function jplopsoft_multiEditorClose(rec,force){
  if(!rec)return;
  if(rec.dirty&&!force&&!await jplopsoft_exosConfirm('「'+rec.name+'」有尚未儲存的變更。確定關閉嗎？'))return;
...
function jplopsoft_openTextCsvMultiEditor(nodeId,name,fmt,source){
  ...
  if(rec.fmt==='csv'){
    try{
      ...
      jplopsoft_exosMessage(e.message);return null;
    }
  }
  ...
  rec.hwnd=jplopsoft_CreateWindowEx(
    jplopsoft_WS_EX_APPWINDOW,
    'ExOS.MultiEditor',
    (rec.fmt==='csv'?'CSV 試算表':(rec.fmt==='xsh'?'XSH 程式編輯器':'文字編輯器'))+' - '+rec.name,
    jplopsoft_WS_OVERLAPPEDWINDOW|jplopsoft_WS_VISIBLE,
  ...
  if(rec.fmt==='txt'||rec.fmt==='xsh')setTimeout(function(){var t=jplopsoft_el(rec.textId);if(t)t.focus();},0);
  return rec;
}
...
function jplopsoft_locateSearchResult(id){
  ...
  setTimeout(function(){
    rows=(jplopsoft_el('jplopsoft_fileRows')&&jplopsoft_el('jplopsoft_fileRows').querySelectorAll)?jplopsoft_el('jplopsoft_fileRows').querySelectorAll('tr[data-node-id]'):[];
    for(i=0;i<rows.length;i++){
      row=rows[i];
    ...
    (function(item,index){
      ...
      row.ondblclick=function(){
        ...
        if(sn&&!jplopsoft_nodeOnlinePreviewable(sn,item.name)){
          jplopsoft_exosMessage(jplopsoft_nodeIsLargeFile(sn)?jplopsoft_largeFileDownloadOnlyMessage(item.name,sn.original_size):'此檔案超過線上預覽安全上限，只允許下載。');
        }else if(f!=='binary'){
...
function jplopsoft_bindFileSearch(){
  ...
  document.addEventListener('click',function(e){
    var ew=jplopsoft_explorerCurrentWindow(),box=ew&&ew.querySelector?ew.querySelector('.jplopsoft_searchbox'):null;
...
function jplopsoft_createDesktopShortcut(targetId){
  ...
  jplopsoft_api(
    ...
    function(err,out){
      if(err){
        jplopsoft_exosMessage(err.message);
        return;
      }
...
async function jplopsoft_deleteDesktopShortcut(targetId){
  var id=parseInt(targetId,10)||0,
      n=jplopsoft_findNode(id),
  ...
  if(!await jplopsoft_exosConfirm(
    '要從桌面刪除「'+name+'」捷徑嗎？\n\n'+
    '只會刪除捷徑，不會刪除原始檔案或資料夾。'
  )){
    return;
...
function jplopsoft_exfsContextPreviewNode(id){
  ...
  if(jplopsoft_nodeIsLargeFile(n)){
    jplopsoft_exosMessage(jplopsoft_largeFileDownloadOnlyMessage(name,n.original_size));
    return;
  }
  if(fmt==='binary'){
    jplopsoft_exosMessage('此 Binary 檔案不提供內容預覽。');
    return;
  }
...
function jplopsoft_exfsContextEditNode(id){
  ...
  if(n.has_motw){
    jplopsoft_exosMessage('此檔案包含 Zone.Identifier (Mark of the Web)，目前只能先以 Low Integrity 檢視器開啟。');
    return;
  }
  if(jplopsoft_nodeIsLargeFile(n)){
    jplopsoft_exosMessage(jplopsoft_largeFileDownloadOnlyMessage(name,n.original_size));
  ...
  if(!jplopsoft_nodeOnlineEditable(n,name)){
    jplopsoft_exosMessage('此檔案超過 18 MiB 線上編輯上限，只允許下載。');
    return;
  }
  if(fmt!=='html'&&fmt!=='txt'&&fmt!=='csv'&&fmt!=='xsh'){
    jplopsoft_exosMessage('此檔案類型不可編輯。');
    return;
  }
  ...
  if(fmt==='xsh'){
    menu.appendChild(
      jplopsoft_exfsContextMenuButton(
        '⚙️',
        '執行 XSH',
        function(){jplopsoft_runXshNode(n.id,'',null);},
        false,
        false
      )
    );
...
function jplopsoft_refreshVisibleFileRowSelection(){
  var host=jplopsoft_el('jplopsoft_fileRows'),rows=host&&host.querySelectorAll?host.querySelectorAll('tr.jplopsoft_item-row'):[],
...
function jplopsoft_moveIdsToFolder(ids,target){
  ...
  if(!jplopsoft_isWritableProfileFolder(target)){jplopsoft_exosMessage('目的地唯讀。只能移動到 C:\\Users\\'+state.samUsername+'\\ 內。');return;}
  if(jplopsoft_invalidMoveTarget(target,ids)){jplopsoft_exosMessage('目的資料夾無效：不能把資料夾移到自己或自己的子資料夾中。');return;}
  for(i=0;i<ids.length;i++){n=jplopsoft_findNode(ids[i]);if(!n||n.parent_id!==target){allSame=false;break;}}
  if(allSame){jplopsoft_setStatus('項目原本就位於該資料夾。');return;}
  conflict=jplopsoft_moveNameConflict(target,ids);if(conflict){jplopsoft_exosMessage('目的資料夾已經有同名項目：「'+conflict+'」。');return;}
  if(ids.length===1){n=jplopsoft_findNode(ids[0]);if(!n)return;jplopsoft_api('move_node','POST',{id:n.id,target_parent_id:target,name_enc:n.name_enc},true,function(err){if(err)return jplopsoft_exosMessage(err.message);state.selectedId=0;jplopsoft_clearChecked();jplopsoft_reloadNodes(function(){jplopsoft_setStatus('拖曳移動完成：1 個項目。');});});}else{jplopsoft_api('move_many','POST',{ids:ids,target_parent_id:target},true,function(err,out){if(err)return jplopsoft_exosMessage(err.message);state.selectedId=0;jplopsoft_clearChecked();jplopsoft_reloadNodes(function(){jplopsoft_setStatus('拖曳移動完成：'+(out&&typeof out.moved==='number'?out.moved:ids.length)+' 個項目。');});});}
}
  ...
  for(i=0;i<list.length;i++){
    ...
    (function(id,row,type,format){
      ...
      row.ondblclick=function(ev){
        ...
        if(!dn){
          jplopsoft_exosMessage('重新導向點的目標不存在或形成循環。');
          ...
          jplopsoft_exosMessage(jplopsoft_nodeIsLargeFile(dn)?jplopsoft_largeFileDownloadOnlyMessage(dname,dn.original_size):'此檔案超過線上預覽安全上限，只允許下載。');
...
function jplopsoft_reloadNodes(done){
  ...
  jplopsoft_api(
    ...
    function(err,out){
      ...
      jplopsoft_explorerRenderAllInstances();

      if(origin&&jplopsoft_explorerInstance(origin)){
        jplopsoft_explorerSelectInstance(origin);
      }

      if(done){
        if(origin&&jplopsoft_explorerInstance(origin)){
          jplopsoft_explorerRunInInstance(origin,done);
        }else{
          done();
        }
      }
...
function jplopsoft_saveFileBlob(name,src){var ext=jplopsoft_fileExtension(name),fmt=jplopsoft_fileFormatFromName(name);name=jplopsoft_validateName(name,'file',ext||(fmt==='html'?'html':(fmt==='csv'?'csv':'txt')));if(!name)return;var mime=fmt==='html'?'text/html;charset=utf-8':(fmt==='csv'?'text/csv;charset=utf-8':'text/plain;charset=utf-8'),blob;try{blob=new Blob([String(src||'')],{type:mime});}catch(e){jplopsoft_exosMessage('瀏覽器無法建立下載檔案：'+e.message);return;}jplopsoft_saveBlobObject(name,blob);}
function jplopsoft_saveBlobObject(name,blob){if(window.navigator&&window.navigator.msSaveOrOpenBlob){window.navigator.msSaveOrOpenBlob(blob,name);return;}var URLObj=window.URL||window.webkitURL,a=document.createElement('a'),url='';if(!URLObj||!URLObj.createObjectURL){jplopsoft_exosMessage('瀏覽器不支援檔案下載。');return;}url=URLObj.createObjectURL(blob);a.href=url;a.download=name;a.style.display='none';document.body.appendChild(a);try{a.click();}catch(e){window.open(url,'_blank');}setTimeout(function(){try{document.body.removeChild(a);}catch(e2){}try{URLObj.revokeObjectURL(url);}catch(e3){}},1200);}
...
function jplopsoft_saveBinaryBlob(name,bytes){var arr;try{arr=(typeof Uint8Array!=='undefined')?new Uint8Array(bytes):bytes;jplopsoft_saveBlobObject(name,new Blob([arr],{type:jplopsoft_mimeForName(name)}));}catch(e){jplopsoft_exosMessage('建立 Binary 下載檔失敗：'+e.message);}}
function jplopsoft_downloadNode(id){
  if(!state.vaultKey)return jplopsoft_exosMessage('請先解密。');var n=jplopsoft_findNode(id),name,fmt;if(!n||n.type!=='file')return jplopsoft_exosMessage('請選擇文件。');name=jplopsoft_decName(n);if(name===null)return jplopsoft_exosMessage('檔名無法解密。');if(jplopsoft_nodeIsLargeFile(n)){jplopsoft_downloadLargeFile(n,name);return;}fmt=jplopsoft_fileFormatFromName(name);jplopsoft_setStatus('正在載入「'+jplopsoft_htmlEscape(name)+'」的加密內容…');jplopsoft_fetchNodeContent(id,function(err,out){var payload,fek;if(err)return jplopsoft_exosMessage(err.message);try{fek=jplopsoft_nodeFekById(id);if(jplopsoft_binaryFormat(fmt)){payload=jplopsoft_decBinaryCipher(out.content_enc,fek);if(payload===null)return jplopsoft_exosMessage('Binary 內容無法解密。');jplopsoft_saveBinaryBlob(name,payload);}else{payload=jplopsoft_decContentCipher(out.content_enc,fek);if(payload===null)return jplopsoft_exosMessage('文件內容無法解密。');jplopsoft_saveFileBlob(name,payload);}jplopsoft_setStatus('已下載「'+jplopsoft_htmlEscape(name)+'」。');}catch(e){jplopsoft_exosMessage(e.message);}},null,'DOWNLOAD');
}
function jplopsoft_downloadSelected(){var n=jplopsoft_findNode(state.selectedId);if(!n)return jplopsoft_exosMessage('請先選取要下載的文件。');if(n.type!=='file')return jplopsoft_exosMessage('資料夾不能直接下載。');jplopsoft_downloadNode(n.id);}
...
function jplopsoft_exfsPrintFrame(title,bodyHtml,extraCss,waitForImage){
  ...
  if(!win||!win.document){
    ...
    jplopsoft_exosMessage('瀏覽器無法建立列印頁面。');
    return;
  }
  ...
  try{
    ...
    jplopsoft_exfsPrintRemoveFrame(frame);
    jplopsoft_exosMessage('建立列印內容失敗：'+e.message);
    return;
  }
  ...
  function jplopsoft_doPrint(){
    ...
    try{
      ...
      jplopsoft_exfsPrintRemoveFrame(frame);
      jplopsoft_exosMessage('無法開啟列印視窗：'+e2.message);
    }
  }
  ...
  if(waitForImage){
    ...
    if(img&&!img.complete){
      ...
      img.onerror=function(){
        jplopsoft_exfsPrintRemoveFrame(frame);
        jplopsoft_exosMessage('圖片尚未完成載入，無法列印。');
      };
      return;
    }
  }
...
/*
 * XSH browser print spooler.
 *
 * Security boundary:
 * - XSH never receives DOM/iframe/Window/printer handles.
 * - The GDI printer DC submits rasterized image pages only.
 * - Only this host-side function calls the browser print engine.
 */
function jplopsoft_xshSpoolPrintJob(ctx,job){
  return new Promise(function(resolve,reject){
    var spec=job&&typeof job==='object'?job:{},pages=Array.isArray(spec.pages)?spec.pages:[],frame,win,doc,html=[],i,p,title,paper,orientation,widthMm,heightMm,totalChars=0,done=false,loaded=0,timer=0;

    function fail(message){
      if(done)return;
      done=true;
      if(timer){try{window.clearTimeout(timer);}catch(ignoreTimer){}}
      if(frame)jplopsoft_exfsPrintRemoveFrame(frame);
      reject(jplopsoft_xshError(jplopsoft_STATUS_INVALID_PARAMETER,String(message||'Print spooler failed.')));
    }
    function cleanup(){
      if(timer){try{window.clearTimeout(timer);}catch(ignoreTimer2){}timer=0;}
      if(frame)jplopsoft_exfsPrintRemoveFrame(frame);
    }
    ...
    for(i=0;i<pages.length;i++){
      p=pages[i]||{};
      if(!/^data:image\/(?:png|jpeg|webp);base64,/i.test(String(p.dataUrl||''))){fail('Print spool accepts PNG, JPEG or WebP Base64 image pages only.');return;}
      totalChars+=String(p.dataUrl||'').length;
      if(String(p.dataUrl||'').length>48*1024*1024){fail('A print page exceeds the 48 MiB encoded spool limit.');return;}
    }
    if(totalChars>384*1024*1024){fail('Print job exceeds the 384 MiB encoded spool limit.');return;}

    title=String(spec.title||'ExOS Print Job').substring(0,240);
    paper=String(spec.paper||'A4').toUpperCase();
    orientation=String(spec.orientation||'portrait').toLowerCase()==='landscape'?'landscape':'portrait';
    widthMm=Math.max(20,Math.min(500,Number(spec.widthMm)||210));
    heightMm=Math.max(20,Math.min(500,Number(spec.heightMm)||297));

    frame=document.createElement('iframe');
    frame.setAttribute('aria-hidden','true');
    frame.setAttribute('title','ExOS XSH print spool buffer');
    /* No scripts are allowed in the spool document. allow-modals is required for print(). */
    frame.setAttribute('sandbox','allow-same-origin allow-modals');
    frame.style.cssText='position:fixed;left:-100000px;top:0;width:1px;height:1px;border:0;opacity:0;pointer-events:none;';
    document.body.appendChild(frame);

    win=frame.contentWindow;
    doc=win&&win.document?win.document:null;
    if(!win||!doc){fail('Unable to create ExOS print spool buffer.');return;}

    html.push('<!doctype html><html><head><meta charset="utf-8"><title>'+jplopsoft_htmlEscape(title)+'</title><style>');
    html.push('@page{size:'+widthMm+'mm '+heightMm+'mm;margin:0;}');
    html.push('html,body{margin:0;padding:0;background:#fff;color:#000;}');
    html.push('.exos-print-page{box-sizing:border-box;width:'+widthMm+'mm;height:'+heightMm+'mm;margin:0;display:flex;align-items:center;justify-content:center;overflow:hidden;page-break-after:always;break-after:page;background:#fff;}');
    html.push('.exos-print-page:last-child{page-break-after:auto;break-after:auto;}');
    html.push('.exos-print-page img{display:block;max-width:100%;max-height:100%;object-fit:contain;}');
    html.push('@media print{html,body{width:auto;height:auto}.exos-print-page{margin:0;box-shadow:none;}}');
    html.push('</style></head><body>');
    for(i=0;i<pages.length;i++){
      p=pages[i]||{};
      html.push('<section class="exos-print-page"><img data-exos-print-page="'+(i+1)+'" alt="Print page '+(i+1)+'" src="'+jplopsoft_htmlEscape(String(p.dataUrl||''))+'"></section>');
    }
    html.push('</body></html>');
    ...
    function invokePrint(){
      if(done)return;
      if(timer){try{window.clearTimeout(timer);}catch(ignoreReadyTimer){}timer=0;}
      try{
        if(win.focus)win.focus();
        if(win.addEventListener){
          win.addEventListener('afterprint',function(){
            finish({ok:true,submitted:true,jobId:Number(spec.jobId)||0,pages:pages.length,title:title,note:'Browser print dialog closed; ExOS cannot determine whether the user printed or cancelled.'});
          },false);
        }
        /* The physical printer / PDF backend is intentionally delegated to the browser. */
        win.print();
        /* Some Chromium builds return from print() without a reliable afterprint event. */
        window.setTimeout(function(){
          finish({ok:true,submitted:true,jobId:Number(spec.jobId)||0,pages:pages.length,title:title,note:'Print job was submitted to the browser print engine.'});
        },250);
      }catch(e2){fail('Browser print engine rejected the spool job: '+String(e2&&e2.message?e2.message:e2));}
    }

    var imgs=doc.getElementsByTagName('img');
    if(!imgs.length){fail('Print spool contains no raster pages.');return;}
    function oneReady(){loaded++;if(loaded>=imgs.length)window.setTimeout(invokePrint,30);}
    for(i=0;i<imgs.length;i++){
      if(imgs[i].complete&&imgs[i].naturalWidth>0){oneReady();continue;}
      imgs[i].onload=oneReady;
      imgs[i].onerror=function(){fail('A raster page failed to load into the print spool buffer.');};
    }
    timer=window.setTimeout(function(){fail('Timed out while preparing print pages.');},15000);
  });
}
...
function jplopsoft_printCurrentDocument(){
  ...
  if(!n||n.type!=='file'){
    jplopsoft_exosMessage('目前沒有可列印的文件。');
    return;
  }
  ...
  if(fmt==='binary'||fmt==='audio'||fmt==='video'||fmt==='rawvideo'){
    jplopsoft_exosMessage('影音 / Binary 檔案沒有適合直接列印的內容。');
    return;
  }
  ...
  if(fmt==='image'){
    ...
    if(!imgSrc){
      jplopsoft_exosMessage('圖片尚未完成載入。');
      return;
    }
  ...
  if(src===null||src===undefined){
    jplopsoft_exosMessage('文件內容尚未載入。');
    return;
  }
...
function jplopsoft_uploadFiles(fileList){
  ...
  function jplopsoft_finishAll(){
    ...
    jplopsoft_reloadNodes(function(){
      ...
      if(skipped.length)jplopsoft_exosMessage(msg+'。\n\n'+skipped.join('\n'));
    });
  }
...
async function jplopsoft_createItem(type,fileFormat){
  ...
  var isFolder=type==='folder',fmt=String(fileFormat||'html').toLowerCase(),raw,name,content='',initial='',actualFmt,fek='',fekWrap='';
  if(fmt!=='csv'&&fmt!=='txt'&&fmt!=='xsh')fmt='html';

  raw=await jplopsoft_exosPrompt(
    isFolder?'新資料夾名稱：':(
      fmt==='csv'?'新 CSV 文件名稱：':(
        fmt==='xsh'?'新 XSH 沙盒程式名稱：':(
          fmt==='txt'?'新文字文件名稱（可輸入 .txt/.ini/.conf/.xml/.asp/.js/.php 等文字副檔名）：':'新 HTML 文件名稱：'
        )
      )
    ),
    isFolder?'新資料夾':(fmt==='csv'?'新文件.csv':(fmt==='xsh'?'hello.xsh':(fmt==='txt'?'新文件.txt':'新文件.html')))
  );
  if(raw===null)return;

  name=jplopsoft_validateName(raw,type,fmt);
  if(!name)return;
  if(jplopsoft_siblingNameExists(state.currentFolder,name,0))return jplopsoft_exosMessage('同一資料夾已有相同名稱。');

  actualFmt=type==='file'?jplopsoft_fileFormatFromName(name):'';
  if(type==='file'&&(actualFmt==='image'||actualFmt==='binary'||actualFmt==='audio'||actualFmt==='video'||actualFmt==='rawvideo')){
    return jplopsoft_exosMessage('Binary / 圖片 / 影音 / XVC 容器請使用專用功能或「上傳文件」匯入。');
  }
  ...
  try{
    ...
    jplopsoft_api('create','POST',{
      ...
      content_enc:content,
      fek_wrap:fekWrap,
      original_size:(type==='file'?jplopsoft_utf8ByteLength(initial):0)
    },true,function(err,out){
      if(err)return jplopsoft_exosMessage(err.message);
      state.selectedId=out.id;
      jplopsoft_reloadNodes(function(){
        if(type==='file')jplopsoft_openEditor(out.id);
      });
    });
  }catch(e){
    jplopsoft_exosMessage(e.message);
  }
}
async function jplopsoft_renameSelected(){var n=jplopsoft_findNode(state.selectedId);if(!n)return jplopsoft_exosMessage('請先選取一個項目。');var old=jplopsoft_decName(n);if(old===null)return jplopsoft_exosMessage('檔名無法解密。');var raw=await jplopsoft_exosPrompt('新的名稱：',old);if(raw===null)return;var preferred=n.type==='file'?jplopsoft_fileExtension(old):'',name=jplopsoft_validateName(raw,n.type,preferred),of,nf,ob,nb;if(!name)return;if(n.type==='file'){of=jplopsoft_fileFormatFromName(old);nf=jplopsoft_fileFormatFromName(name);ob=(of==='image'||of==='binary');nb=(nf==='image'||nf==='binary');if(ob!==nb)return jplopsoft_exosMessage('不可直接在「文字/HTML」與「Binary」兩種內容類別間變更副檔名。');}if(jplopsoft_siblingNameExists(n.parent_id,name,n.id))return jplopsoft_exosMessage('同一資料夾已有相同名稱。');try{jplopsoft_api('rename','POST',{id:n.id,name_enc:jplopsoft_encName(name)},true,function(err){if(err)return jplopsoft_exosMessage(err.message);jplopsoft_reloadNodes();});}catch(e){jplopsoft_exosMessage(e.message);}}
...
function jplopsoft_openMoveDialog(){
  ...
  if(!ids.length){
    jplopsoft_exosMessage('請先勾選檔案/資料夾，或點選一個項目。');
    return;
  }
...
async function jplopsoft_confirmMove(){
  ...
  if(jplopsoft_invalidMoveTarget(target,ids)){
    jplopsoft_exosMessage('目的資料夾無效：不能把資料夾移到自己或自己的子資料夾中。');
    return;
  }
  ...
  if(allSame){
    jplopsoft_exosMessage('這些項目原本就已經在「'+jplopsoft_moveTargetName(target)+'」。');
    return;
  }

  conflict=jplopsoft_moveNameConflict(target,ids);
  if(conflict){
    jplopsoft_exosMessage('目的資料夾已經有同名項目：「'+conflict+'」。\n\n請先重新命名其中一個項目。');
    return;
  }

  msg='確定將 '+ids.length+' 個項目移動到「'+jplopsoft_moveTargetName(target)+'」嗎？\n\n移動只修改 flat-file parent_id；既有 .x6f 與版本檔不會搬動或重新加密。';
  if(!await jplopsoft_exosConfirm(msg))return;
...
function jplopsoft_refreshTrash(done){
  jplopsoft_api('trash_list','GET',null,true,function(err,out){
    if(err){
      if(done)done(err);
      else jplopsoft_exosMessage(err.message);
      return;
    }
...
async function jplopsoft_restoreTrashItem(id){
...
async function jplopsoft_permanentDeleteTrashItem(id,name){
  if(!await jplopsoft_exosConfirm(
...
async function jplopsoft_emptyTrash(){
  ...
  if(!await jplopsoft_exosConfirm('最後確認：真的要永久清空資源回收桶？此操作無法復原。'))return;
...
async function jplopsoft_deleteSelected(){var ids=[],k,n,name,msg,folders=0,files=0;if(jplopsoft_checkedCount()>0){for(k in state.checkedIds)if(state.checkedIds.hasOwnProperty(k)&&state.checkedIds[k])ids.push(parseInt(k,10));for(k=0;k<ids.length;k++){n=jplopsoft_findNode(ids[k]);if(n&&n.type==='folder')folders++;else if(n)files++;}msg='確定將已勾選的 '+ids.length+' 個項目移到垃圾桶嗎？';if(folders>0)msg+='\n\n其中包含 '+folders+' 個資料夾；資料夾內全部內容會一起進垃圾桶。';if(files>0)msg+='\n檔案：'+files+' 個';msg+='\n\n此時不會刪除 .x6f，可以從垃圾桶還原。';if(!await jplopsoft_exosConfirm(msg))return;jplopsoft_api('delete_many','POST',{ids:ids},true,function(err,out){if(err)return jplopsoft_exosMessage(err.message);state.selectedId=0;jplopsoft_clearChecked();jplopsoft_reloadNodes(function(){jplopsoft_setStatus('已將 '+(out.trashed||ids.length)+' 個項目移到垃圾桶。');});});return;}n=jplopsoft_findNode(state.selectedId);if(!n)return jplopsoft_exosMessage('請先勾選檔案，或點選一個項目。');name=jplopsoft_decName(n)||('#'+n.id);msg=n.type==='folder'?'確定將資料夾「'+name+'」以及其中全部內容移到垃圾桶嗎？':'確定將「'+name+'」移到垃圾桶嗎？';msg+='\n\n此時不會刪除 .x6f，可以從垃圾桶還原。';if(!await jplopsoft_exosConfirm(msg))return;jplopsoft_api('delete','POST',{id:n.id},true,function(err){if(err)return jplopsoft_exosMessage(err.message);state.selectedId=0;jplopsoft_reloadNodes(function(){jplopsoft_setStatus('已將「'+jplopsoft_htmlEscape(name)+'」移到垃圾桶。');});});}
...
async function jplopsoft_richImageProperties(){
  var img=jplopsoft_richSelectedImage(),alt,width,n;
  if(!img)return jplopsoft_exosMessage('請先在即時編輯區點選一張圖片。');
  alt=await jplopsoft_exosPrompt('圖片替代文字（alt）：',String(img.getAttribute('alt')||''));if(alt===null)return;
  width=await jplopsoft_exosPrompt('圖片寬度：可留空自動，或輸入 640、50%、640px。',String(img.style.width||img.getAttribute('width')||''));if(width===null)return;
  width=String(width||'').replace(/^\s+|\s+$/g,'');
  if(width!==''&&!/^\d{1,4}(?:px)?$/.test(width)&&!/^\d{1,3}%$/.test(width))return jplopsoft_exosMessage('寬度格式錯誤。請輸入 640、640px、50%，或留空。');
  if(/%$/.test(width)){n=parseInt(width,10);if(n<1||n>100)return jplopsoft_exosMessage('百分比寬度需為 1%～100%。');}
  else if(width!==''){n=parseInt(width,10);if(n<1||n>4000)return jplopsoft_exosMessage('像素寬度需為 1～4000。');width=n+'px';}
...
function jplopsoft_richInsertImageFiles(files){
  ...
  if(files.length>jplopsoft_HTML_EMBED_IMAGE_BATCH_MAX)jplopsoft_exosMessage('一次最多插入 '+jplopsoft_HTML_EMBED_IMAGE_BATCH_MAX+' 張圖片，其餘未處理。');
  function next(err){
    if(err){jplopsoft_exosMessage(err.message||String(err));return;}
...
function jplopsoft_csvAddRow(){
  ...
  if(state.csvData.length>=jplopsoft_CSV_GRID_MAX_ROWS)return jplopsoft_exosMessage('CSV 試算表最多 '+jplopsoft_CSV_GRID_MAX_ROWS+' 列。');
...
function jplopsoft_csvAddColumn(){
  ...
  if(state.csvData.length&&state.csvData[0].length>=jplopsoft_CSV_GRID_MAX_COLS)return jplopsoft_exosMessage('CSV 試算表最多 '+jplopsoft_CSV_GRID_MAX_COLS+' 欄。');
...
function jplopsoft_csvDeleteRow(){
  if(state.csvReadOnly)return;if(state.csvData.length<=1)return jplopsoft_exosMessage('至少需要保留 1 列。');
...
function jplopsoft_csvDeleteColumn(){
  ...
  cols=state.csvData.length&&state.csvData[0]?state.csvData[0].length:0;if(cols<=1)return jplopsoft_exosMessage('至少需要保留 1 欄。');
...
function jplopsoft_openImagePreview(id){var n=jplopsoft_resolveClientNode(id),name=n?jplopsoft_decName(n):'';if(!n||n.type!=='file'||jplopsoft_fileFormatFromName(name)!=='image')return;jplopsoft_launchSystemXshApp('image_viewer',[jplopsoft_cmdNodeFullPath(n)]);}
  ...
  jplopsoft_fetchNodeContent(
    ...
    function(err,out){
      ...
      try{
        ...
        if(bytes===null){
          return jplopsoft_exosMessage('影音內容無法解密。');
        }

        typed=jplopsoft_binaryBytesToUint8(bytes);
      }catch(e){
        return jplopsoft_exosMessage(e.message);
      }
      ...
      try{
        ...
        return jplopsoft_exosMessage('影音預覽失敗：'+e2.message);
      }
...
function jplopsoft_openEditor(id,routeInternal){
  ...
  jplopsoft_fetchNodeContent(
    ...
    function(err,out){
      ...
      if(src===null){
        return jplopsoft_exosMessage('文件內容無法解密。');
      }
...
function jplopsoft_openView(id,routeInternal){
  ...
  if(jplopsoft_nodeIsLargeFile(n))return jplopsoft_exosMessage(jplopsoft_largeFileDownloadOnlyMessage(name,n.original_size));
  if(!jplopsoft_nodeOnlinePreviewable(n,name))return jplopsoft_exosMessage('此檔案超過線上預覽安全上限，只允許下載。');
  if(fmt==='image')return jplopsoft_exosMessage('此圖片缺少必要的 JPEG128_V1 加密縮圖，因此不允許載入原圖預覽。');
  if(fmt==='audio'||fmt==='video'||fmt==='rawvideo')return jplopsoft_openMediaPreview(id);
  if(fmt==='binary')return jplopsoft_exosMessage('此 Binary 檔案不提供內容預覽，請使用下載。');
  jplopsoft_setStatus('正在載入「'+jplopsoft_htmlEscape(name||('#'+id))+'」的加密內容…');
  jplopsoft_fetchNodeContent(id,function(err,out){
    var src;
    if(err)return jplopsoft_exosMessage(err.message);
    try{src=jplopsoft_decContentCipher(out.content_enc,jplopsoft_nodeFekById(id));}catch(e){return jplopsoft_exosMessage(e.message);}
    if(src===null)return jplopsoft_exosMessage('文件內容無法解密。');
    ...
    if(fmt==='csv'){
      ...
      try{jplopsoft_csvLoadEditor(src,true);}catch(csvErr){jplopsoft_closeModal();return jplopsoft_exosMessage(csvErr.message);}
    ...
    if(fmt==='txt'||fmt==='xsh'){state.editorMode='source';jplopsoft_el('jplopsoft_htmlEditor').readOnly=true;jplopsoft_el('jplopsoft_htmlEditor').spellcheck=(fmt==='txt');jplopsoft_el('jplopsoft_htmlEditor').className='jplopsoft_editor';}
...
function jplopsoft_saveDocument(){if(!state.editId)return;if(state.editorMode==='rich'&&state.openFormat==='html')jplopsoft_syncRichToSource();var src=state.openFormat==='csv'?jplopsoft_csvCurrentSource():jplopsoft_el('jplopsoft_htmlEditor').value,plainSize=jplopsoft_utf8ByteLength(src);if(plainSize>jplopsoft_TEXT_ONLINE_EDIT_MAX)return jplopsoft_exosMessage('線上編輯內容超過 18 MiB。請改用大型檔案上傳模式；大型 TXT / HTML / CSV 僅允許下載。');try{var cipher=jplopsoft_encContent(src,jplopsoft_nodeFekById(state.editId));jplopsoft_saveNodeCipher(state.editId,cipher,plainSize,function(err,out){if(err)return jplopsoft_exosMessage(err.message);jplopsoft_closeModal();jplopsoft_reloadNodes(function(){jplopsoft_setStatus('文件已以原檔 FEK 加密保存為版本 v'+out.version_no+'。');});});}catch(e){jplopsoft_exosMessage(e.message);}}
...
function jplopsoft_versionWindowSetMaximized(on){
  jplopsoft_wmSetOverlayMaximized('jplopsoft_versionWindow',!!on);
}

function jplopsoft_versionWindowToggleMax(){
  jplopsoft_wmToggleOverlayMax('jplopsoft_versionWindow');
  jplopsoft_wmActivateOverlay(
    'jplopsoft_versionBackdrop',
    'jplopsoft_versionWindow',
    'versions'
  );
}

function jplopsoft_versionWindowMinimize(){
...
function jplopsoft_versionWindowRestore(){
  jplopsoft_wmOverlayRestore(
    'jplopsoft_versionBackdrop',
    'jplopsoft_versionWindow',
    'versions'
  );
...
function jplopsoft_closeVersions(){
  var b=jplopsoft_el('jplopsoft_versionBackdrop');

  if(b)b.style.display='none';

  jplopsoft_versionWindowSetMaximized(false);
  jplopsoft_taskbarRemoveApp('versions');

  state.versionList=[];
  state.versionCurrentNo=0;

  jplopsoft_revokeVersionPreview();

  jplopsoft_el('jplopsoft_versionRows').innerHTML='';
  jplopsoft_el('jplopsoft_versionTextPreview').className=
    'jplopsoft_version-text jplopsoft_hidden';
  jplopsoft_el('jplopsoft_versionHtmlPreview').className=
    'jplopsoft_preview jplopsoft_hidden';
  jplopsoft_el('jplopsoft_versionHtmlPreviewIE11').className=
    'jplopsoft_ie11-html-preview jplopsoft_hidden';
  jplopsoft_el('jplopsoft_versionImagePreview').className=
    'jplopsoft_version-image jplopsoft_hidden';
  jplopsoft_el('jplopsoft_versionImagePreview').removeAttribute('src');
  jplopsoft_el('jplopsoft_versionPreviewEmpty').className=
    'jplopsoft_version-empty';
  jplopsoft_el('jplopsoft_versionPreviewTitle').textContent=
    '選擇一個版本即可檢視。';
}
...
function jplopsoft_viewVersion(versionId,versionNo){if(state.openFormat==='image')return jplopsoft_exosMessage('圖片版本不直接解密原圖預覽；請使用主畫面的 128 × 128 加密縮圖。版本原圖仍可使用「下載」。');jplopsoft_fetchVersion(versionId,function(err,v,src){if(err)return jplopsoft_exosMessage(err.message);jplopsoft_renderVersionPreview(versionNo,src);});}
function jplopsoft_downloadVersion(versionId,versionNo){jplopsoft_fetchVersion(versionId,function(err,v,src){if(err)return jplopsoft_exosMessage(err.message);var n=jplopsoft_findNode(state.openId),name=n?jplopsoft_decName(n):'document',vn=jplopsoft_versionDownloadName(name||'document',versionNo),fmt=jplopsoft_fileFormatFromName(name||'');if(jplopsoft_binaryFormat(fmt))jplopsoft_saveBinaryBlob(vn,src);else jplopsoft_saveFileBlob(vn,src);});}
async function jplopsoft_restoreVersion(versionId,versionNo){if(jplopsoft_binaryFormat(state.openFormat))return jplopsoft_exosMessage('Binary / 圖片 / 影音版本目前不提供文字內容還原。');if(!state.editId)return jplopsoft_exosMessage('請用「編輯」模式開啟文件後再還原版本。');if(!await jplopsoft_exosConfirm('確定把版本 v'+versionNo+' 還原成目前內容嗎？\\n\\n還原本身會再建立一個新的版本，因此原歷史不會消失。'))return;var nodeId=state.openId;jplopsoft_fetchVersion(versionId,function(err,v,src){if(err)return jplopsoft_exosMessage(err.message);var cipher;try{cipher=jplopsoft_encContent(src,jplopsoft_nodeFekById(nodeId));}catch(e){return jplopsoft_exosMessage(e.message);}jplopsoft_saveNodeCipher(nodeId,cipher,jplopsoft_utf8ByteLength(src),function(err2,out){if(err2)return jplopsoft_exosMessage(err2.message);jplopsoft_closeVersions();jplopsoft_reloadNodes(function(){jplopsoft_openEditor(nodeId);jplopsoft_setStatus('已將 v'+versionNo+' 還原並以原檔 FEK 另存為新版本 v'+out.version_no+'。');});});});}
...
function jplopsoft_openVersions(){
  var n,name;

  if(!state.openId)return jplopsoft_exosMessage('請先打開文件。');

  n=jplopsoft_findNode(state.openId);
  name=n?jplopsoft_decName(n):'';

  if(
    n&&
    jplopsoft_nodeIsLargeFile(n)&&
    jplopsoft_fileFormatFromName(name)!=='image'
  ){
    return jplopsoft_exosMessage(
      '大型 CHUNKED_V1 檔案不提供線上內容版本預覽；'+
      '可重新命名、移動、複製、刪除與下載。'
    );
  }

  jplopsoft_el('jplopsoft_versionTitle').textContent=
    '版本紀錄：'+(name||('#'+state.openId));

  jplopsoft_el('jplopsoft_versionBackdrop').style.display='flex';
  jplopsoft_versionWindowSetMaximized(false);
  ...
  jplopsoft_el('jplopsoft_versionRows').innerHTML='';
  jplopsoft_el('jplopsoft_versionEmpty').className=
    'jplopsoft_version-empty';
  jplopsoft_el('jplopsoft_versionPreviewEmpty').className=
    'jplopsoft_version-empty';
  jplopsoft_el('jplopsoft_versionTextPreview').className=
  ...
  jplopsoft_api(
    ...
    function(err,out){
      if(err){
        ...
        return jplopsoft_exosMessage(err.message);
      }

      state.versionList=out.versions||[];
      state.versionCurrentNo=out.current_version||0;
      jplopsoft_renderVersions();
    }
  );
}
...
function jplopsoft_documentWindowApplyMax(on){
  ...
  m.className=
    'jplopsoft_modal jplopsoft_document-window '+
    'jplopsoft_native-overlay-window jplopsoft_wm-overlay-window'+
    (on?' jplopsoft_maximized jplopsoft_wm-maximized':'');
}
function jplopsoft_toggleMax(){
  state.maximized=!state.maximized;
  jplopsoft_documentWindowApplyMax(state.maximized);
  jplopsoft_wmActivateOverlay(
    'jplopsoft_modalBackdrop',
    'jplopsoft_documentWindow',
    'document'
  );
  if(jplopsoft_isIE11Browser()){
    setTimeout(jplopsoft_ie11FitDocumentModal,0);
...
function jplopsoft_bindRich(){
  ...
  jplopsoft_el('jplopsoft_richFore').onchange=function(){var v=String(this.value||'');if(!/^#[0-9a-f]{6}$/i.test(v)){if(jplopsoft_isIE11Browser())jplopsoft_exosMessage('請輸入 #RRGGBB 格式，例如 #ff0000。');return;}jplopsoft_execRich('foreColor',v);};
  jplopsoft_el('jplopsoft_richBack').onchange=function(){var v=String(this.value||'');if(!/^#[0-9a-f]{6}$/i.test(v)){if(jplopsoft_isIE11Browser())jplopsoft_exosMessage('請輸入 #RRGGBB 格式，例如 #ffff00。');return;}jplopsoft_execRich('hiliteColor',v);jplopsoft_execRich('backColor',v);};
  jplopsoft_el('jplopsoft_richLink').onclick=async function(){var u=await jplopsoft_exosPrompt('連結網址：','https://');if(!u)return;u=String(u).replace(/^\s+|\s+$/g,'');if(/^\s*(javascript|data|vbscript):/i.test(u))return jplopsoft_exosMessage('基於安全性，不允許 javascript: / data: / vbscript: 連結。');jplopsoft_execRich('createLink',u);};
  jplopsoft_el('jplopsoft_richTable').onclick=async function(){var r=parseInt(await jplopsoft_exosPrompt('列數：','3'),10),c=parseInt(await jplopsoft_exosPrompt('欄數：','3'),10),i,j,h='<table><tbody>';if(!(r>0&&r<=30&&c>0&&c<=20))return;for(i=0;i<r;i++){h+='<tr>';for(j=0;j<c;j++)h+='<td>&nbsp;</td>';h+='</tr>';}h+='</tbody></table><p><br></p>';jplopsoft_richInsertHtml(h);jplopsoft_richUpdateImageInfo();};
  if(imageBtn)imageBtn.onclick=function(){jplopsoft_richSaveSelection();if(imageInput){imageInput.value='';try{imageInput.click();}catch(e){jplopsoft_exosMessage('瀏覽器無法開啟圖片選擇器。');}}};
...
function jplopsoft_submitCredentialUI(){
  ...
  if(!state.initialized){
    ...
    if(!raw){
      jplopsoft_exosMessage('請建立 administrator 密碼。');
      jplopsoft_focusDecryptPassword(false);
      return;
    }

    if(raw!==confirm){
      jplopsoft_exosMessage('兩次輸入的密碼不一致。');
...
/* =========================================================================
 * Explorer multi-window architecture - os28
 *
 * Windows Explorer semantics:
 * - Multiple Explorer HWNDs may coexist.
 * - They share one explorer.exe / Shell process.
 * - Each HWND owns an independent navigation/UI state snapshot.
 * - Existing Explorer rendering code remains source-compatible by resolving
 *   base DOM IDs against the active/temporary Explorer instance.
 * ========================================================================= */
var jplopsoft_EXPLORER_INSTANCES={};
var jplopsoft_EXPLORER_SEQ=0;
var jplopsoft_EXPLORER_ACTIVE_ID='';
var jplopsoft_EXPLORER_EXEC_ID='';
var jplopsoft_EXPLORER_STATE_KEYS=[
  'currentFolder','selectedId','desktopSelectedTargetId','desktopShellSelected',
  'checkedIds','checkedFolder','sortKey','sortDir','shiftAnchorId','contextNodeId',
  'searchResults','searchActive','sidebarWidth'
];

function jplopsoft_explorerCloneValue(v){
  var out,k,i;
  if(v===null||typeof v==='undefined')return v;
  if(typeof v!=='object')return v;
  if(Object.prototype.toString.call(v)==='[object Array]'){
    out=[];
    for(i=0;i<v.length;i++)out[i]=jplopsoft_explorerCloneValue(v[i]);
    return out;
  }
  out={};
  for(k in v){
    if(v.hasOwnProperty(k))out[k]=jplopsoft_explorerCloneValue(v[k]);
  }
  return out;
}

function jplopsoft_explorerInstance(id){
  return jplopsoft_EXPLORER_INSTANCES[String(id||'')]||null;
}

function jplopsoft_explorerContextByAppId(appId){
  var k,c;
  appId=String(appId||'');
  for(k in jplopsoft_EXPLORER_INSTANCES){
    if(!jplopsoft_EXPLORER_INSTANCES.hasOwnProperty(k))continue;
    c=jplopsoft_EXPLORER_INSTANCES[k];
    if(c&&c.appId===appId)return c;
  }
  return null;
}

function jplopsoft_explorerContextByWindowId(windowId){
  var k,c;
  windowId=String(windowId||'');
  for(k in jplopsoft_EXPLORER_INSTANCES){
    if(!jplopsoft_EXPLORER_INSTANCES.hasOwnProperty(k))continue;
    c=jplopsoft_EXPLORER_INSTANCES[k];
    if(c&&c.windowId===windowId)return c;
  }
  return null;
}

function jplopsoft_explorerCurrentContext(){
  var id=String(jplopsoft_EXPLORER_EXEC_ID||jplopsoft_EXPLORER_ACTIVE_ID||'1');
  return jplopsoft_explorerInstance(id);
}

function jplopsoft_explorerCurrentWindow(){
  var c=jplopsoft_explorerCurrentContext();
  return c?document.getElementById(c.windowId):document.getElementById('jplopsoft_explorerWindow');
}

function jplopsoft_explorerResolveElement(id){
  var c=jplopsoft_explorerCurrentContext(),mapped;
  id=String(id||'');
  if(c&&c.idMap&&c.idMap[id]){
    mapped=String(c.idMap[id]);
    return document.getElementById(mapped);
  }
  return document.getElementById(id);
}

function jplopsoft_explorerCaptureState(c){
  var i,k;
  if(!c)return;
  if(!c.state)c.state={};
  for(i=0;i<jplopsoft_EXPLORER_STATE_KEYS.length;i++){
    k=jplopsoft_EXPLORER_STATE_KEYS[i];
    c.state[k]=jplopsoft_explorerCloneValue(state[k]);
  }
}

function jplopsoft_explorerLoadState(c){
  var i,k;
  if(!c||!c.state)return;
  for(i=0;i<jplopsoft_EXPLORER_STATE_KEYS.length;i++){
    k=jplopsoft_EXPLORER_STATE_KEYS[i];
    if(typeof c.state[k]!=='undefined'){
      state[k]=jplopsoft_explorerCloneValue(c.state[k]);
    }
  }
}

function jplopsoft_explorerSelectInstance(id){
  var oldc,newc;
  id=String(id||'');
  newc=jplopsoft_explorerInstance(id);
  if(!newc)return false;
  if(jplopsoft_EXPLORER_ACTIVE_ID===id)return true;

  oldc=jplopsoft_explorerInstance(jplopsoft_EXPLORER_ACTIVE_ID);
  if(oldc)jplopsoft_explorerCaptureState(oldc);

  jplopsoft_EXPLORER_ACTIVE_ID=id;
  jplopsoft_explorerLoadState(newc);
  return true;
}

function jplopsoft_explorerSelectByAppId(appId){
  var c=jplopsoft_explorerContextByAppId(appId);
  if(!c)return false;
  return jplopsoft_explorerSelectInstance(c.instanceId);
}

function jplopsoft_explorerRunInInstance(id,fn){
  var target=jplopsoft_explorerInstance(id),
      oldActive=String(jplopsoft_EXPLORER_ACTIVE_ID||''),
      oldExec=String(jplopsoft_EXPLORER_EXEC_ID||''),
      activeCtx,ret;

  if(!target||typeof fn!=='function')return null;

  activeCtx=jplopsoft_explorerInstance(oldActive);
  if(activeCtx)jplopsoft_explorerCaptureState(activeCtx);

  jplopsoft_explorerLoadState(target);
  jplopsoft_EXPLORER_EXEC_ID=String(target.instanceId);

  try{
    ret=fn();
  }finally{
    jplopsoft_explorerCaptureState(target);
    jplopsoft_EXPLORER_EXEC_ID=oldExec;
    if(activeCtx)jplopsoft_explorerLoadState(activeCtx);
  }

  return ret;
}

function jplopsoft_explorerBuildIdentityMap(win){
  var map={},nodes,i,id;
  if(!win)return map;
  id=String(win.id||'');
  if(id)map[id]=id;
  if(!win.querySelectorAll)return map;
  nodes=win.querySelectorAll('[id]');
  for(i=0;i<nodes.length;i++){
...
function jplopsoft_explorerRegisterPrimary(){
  var win,c;
  if(jplopsoft_explorerInstance('1'))return jplopsoft_explorerInstance('1');

  win=document.getElementById('jplopsoft_explorerWindow');
  if(!win)return null;

  jplopsoft_EXPLORER_SEQ=1;
  c={
    instanceId:'1',
    seq:1,
    primary:true,
    ...
    idMap:jplopsoft_explorerBuildIdentityMap(win),
    state:{},
    hwnd:0,
    opened:false,
    visible:false,
  ...
  jplopsoft_EXPLORER_INSTANCES['1']=c;

  if(!jplopsoft_EXPLORER_ACTIVE_ID){
    jplopsoft_EXPLORER_ACTIVE_ID='1';
  }

  return c;
}

function jplopsoft_explorerRemapCloneIds(win,seq){
  var map={},all=[],nodes,i,n,base,newId,suffix='__exp'+String(seq);
  if(!win)return map;

  all.push(win);
  if(win.querySelectorAll){
    nodes=win.querySelectorAll('[id]');
  ...
  for(i=0;i<all.length;i++){
    n=all[i];
    base=String(n.getAttribute&&n.getAttribute('data-explorer-base-id')||n.id||'');
    if(!base)continue;
    newId=base+suffix;
    map[base]=newId;
    if(n.setAttribute)n.setAttribute('data-explorer-base-id',base);
    n.id=newId;
    if(n.removeAttribute){
      n.removeAttribute('data-exfs-hwnd');
      n.removeAttribute('data-exfs-win32-class');
    }
  }
...
function jplopsoft_explorerNode(c,baseId){
  if(!c||!c.idMap)return null;
  return document.getElementById(c.idMap[String(baseId||'')]||'');
}

function jplopsoft_explorerApplySidebarForContext(c){
  var w=jplopsoft_explorerNode(c,'jplopsoft_explorerWindow'),
      aside=w&&w.querySelector?w.querySelector('.jplopsoft_sidebar'):null,
      width=c&&c.state?parseInt(c.state.sidebarWidth,10)||270:270;
  if(width<190)width=190;
  if(width>520)width=520;
  if(aside)aside.style.width=width+'px';
}

function jplopsoft_explorerBindSidebarForContext(c){
  var handle=jplopsoft_explorerNode(c,'jplopsoft_sidebarResizer'),
      win=jplopsoft_explorerNode(c,'jplopsoft_explorerWindow'),
      aside=win&&win.querySelector?win.querySelector('.jplopsoft_sidebar'):null,
      dragging=false,startX=0,startW=0;

  if(!handle||!aside)return;
  jplopsoft_explorerApplySidebarForContext(c);
  ...
  if(document.addEventListener){
    document.addEventListener('mousemove',function(e){
      ...
      if(!dragging)return;
      e=e||window.event;
      w=startW+((typeof e.clientX==='number'?e.clientX:0)-startX);
      if(w<190)w=190;
      if(w>520)w=520;
      aside.style.width=w+'px';
      c.state.sidebarWidth=w;
      if(jplopsoft_EXPLORER_ACTIVE_ID===c.instanceId)state.sidebarWidth=w;
    },false);
    document.addEventListener('mouseup',function(){
      if(!dragging)return;
      dragging=false;
      jplopsoft_explorerRunInInstance(c.instanceId,function(){
        state.sidebarWidth=aside.offsetWidth||state.sidebarWidth||270;
        jplopsoft_uiStorageSave();
      });
    },false);
  }
...
function jplopsoft_explorerBindSearchForContext(c){
  var input=jplopsoft_explorerNode(c,'jplopsoft_fileSearchInput'),
      clear=jplopsoft_explorerNode(c,'jplopsoft_fileSearchClear');

  if(!input||!clear)return;

  input.oninput=function(){
    var value=this.value,id=c.instanceId;
    if(c.searchTimer){
      try{window.clearTimeout(c.searchTimer);}catch(ignoreExplorerSearchTimer){}
    }
    c.searchTimer=window.setTimeout(function(){
      jplopsoft_explorerRunInInstance(id,function(){
        jplopsoft_renderFileSearch(value);
      });
    },120);
  };

  input.onkeydown=function(e){
    var k,id=c.instanceId;
    e=e||window.event;
    k=e.keyCode||e.which;

    jplopsoft_explorerSelectInstance(id);
    ...
    if(k===40){
      if(e.preventDefault)e.preventDefault();
      jplopsoft_updateSearchActive(1);
      return false;
    }
    if(k===38){
      if(e.preventDefault)e.preventDefault();
    ...
    if(k===13&&state.searchActive>=0&&state.searchResults[state.searchActive]){
      if(e.preventDefault)e.preventDefault();
      jplopsoft_locateSearchResult(state.searchResults[state.searchActive].id);
      return false;
    }
  ...
  input.onfocus=function(){
    ...
    if(jplopsoft_trim(input.value))jplopsoft_renderFileSearch(input.value);
  };

  clear.onclick=function(){
    jplopsoft_explorerActivateInstance(c.instanceId,true);
    jplopsoft_clearFileSearch();
    try{input.focus();}catch(ignoreExplorerSearchFocus){}
  };
}

function jplopsoft_explorerBindToolbarForContext(c){
  function bind(id,fn){
    var n=jplopsoft_explorerNode(c,id);
    if(!n)return;
  ...
  bind('jplopsoft_newFolderBtn',function(){jplopsoft_createItem('folder');});
  bind('jplopsoft_newHtmlBtn',function(){jplopsoft_createItem('file','html');});
  bind('jplopsoft_newTxtBtn',function(){jplopsoft_createItem('file','txt');});
  bind('jplopsoft_newCsvBtn',function(){jplopsoft_createItem('file','csv');});
  bind('jplopsoft_newXshBtn',function(){jplopsoft_createItem('file','xsh');});
  bind('jplopsoft_downloadBtn',jplopsoft_downloadSelected);
  bind('jplopsoft_renameBtn',jplopsoft_renameSelected);
  bind('jplopsoft_moveBtn',jplopsoft_openMoveDialog);
  bind('jplopsoft_deleteBtn',function(){
    if(jplopsoft_isDesktopFolder()){
      if(state.desktopSelectedTargetId)jplopsoft_deleteDesktopShortcut(state.desktopSelectedTargetId);
      return;
    }
    jplopsoft_deleteSelected();
  });
  bind('jplopsoft_volume3dBtn',function(){jplopsoft_openVolume3D();});

  var uploadBtn=jplopsoft_explorerNode(c,'jplopsoft_uploadFileBtn'),
      uploadInput=jplopsoft_explorerNode(c,'jplopsoft_uploadFileInput');

  if(uploadBtn&&uploadInput){
    uploadBtn.onclick=function(){
      jplopsoft_explorerActivateInstance(c.instanceId,true);
      if(jplopsoft_isDesktopFolder())return jplopsoft_exosMessage('桌面只存放捷徑，請先進入一般資料夾再上傳。');
      if(!jplopsoft_isWritableProfileFolder(state.currentFolder))return jplopsoft_exosMessage('此位置唯讀。只能上傳到 C:\\Users\\'+state.samUsername+'\\ 內。');
      if(!state.vaultKey)return jplopsoft_exosMessage('請先解鎖。');
      uploadInput.value='';
      uploadInput.click();
    };
    uploadInput.onchange=function(){
      var files=this.files,id=c.instanceId;
      if(files&&files.length){
        jplopsoft_explorerSelectInstance(id);
        jplopsoft_uploadFiles(files);
      }
      this.value='';
    };
  }
...
function jplopsoft_explorerBindInstance(c){
  var win,titlebar;
  if(!c)return;

  win=jplopsoft_explorerNode(c,'jplopsoft_explorerWindow');
  titlebar=jplopsoft_explorerNode(c,'jplopsoft_explorerTitlebar');
  if(!win)return;

  win.setAttribute('data-explorer-instance',String(c.instanceId));
  win.setAttribute('data-explorer-app-id',String(c.appId));
  ...
  if(titlebar){
    titlebar.ondblclick=function(e){
      e=e||window.event;
      if(jplopsoft_wmCanDragTarget(e.target||e.srcElement)){
        jplopsoft_explorerActivateInstance(c.instanceId,true);
        jplopsoft_wmToggleMax(c.windowId);
      }
    };
  }

  jplopsoft_wmMakeDraggable(c.windowId,c.titlebarId);

  jplopsoft_explorerRunInInstance(c.instanceId,function(){
    jplopsoft_explorerBindToolbarForContext(c);
    jplopsoft_bindSortHeaders();
    jplopsoft_bindBulkSelection();
    jplopsoft_bindExfsContextMenu();
    jplopsoft_explorerBindSearchForContext(c);
  });

  jplopsoft_explorerBindSidebarForContext(c);
  jplopsoft_applySvgIcons(win);
}

function jplopsoft_explorerCreateClone(folderId){
  var source=document.getElementById('jplopsoft_explorerWindow'),
      win,c,seq,hwnd,rec;

  if(!source)return null;

  seq=++jplopsoft_EXPLORER_SEQ;
  win=source.cloneNode(true);

  win.removeAttribute('style');
  win.className=String(win.className||'')
    .replace(/\s*jplopsoft_wm-active/g,'')
    .replace(/\s*jplopsoft_wm-maximized/g,'')
    .replace(/\s*jplopsoft_dwm-active/g,'')
    .replace(/\s*jplopsoft_dwm-inactive/g,'');

  c={
    instanceId:String(seq),
    seq:seq,
    primary:false,
    appId:'explorer_'+String(seq),
    idMap:jplopsoft_explorerRemapCloneIds(win,seq),
    state:{},
    hwnd:0,
    opened:true,
    visible:true,
  ...
  c.windowId=c.idMap['jplopsoft_explorerWindow'];
  c.titlebarId=c.idMap['jplopsoft_explorerTitlebar'];
  c.titleId=c.idMap['jplopsoft_explorerWindowTitle'];
  c.minButtonId=c.idMap['jplopsoft_explorerMinBtn'];
  c.maxButtonId=c.idMap['jplopsoft_explorerMaxBtn'];
  c.closeButtonId=c.idMap['jplopsoft_explorerCloseBtn'];

  var active=jplopsoft_explorerInstance(jplopsoft_EXPLORER_ACTIVE_ID);
  if(active)jplopsoft_explorerCaptureState(active);

  c.state=active
    ?jplopsoft_explorerCloneValue(active.state)
    :{};

  c.state.currentFolder=parseInt(folderId,10);
  if(isNaN(c.state.currentFolder))c.state.currentFolder=0;
  c.state.selectedId=0;
  c.state.desktopSelectedTargetId=0;
  c.state.desktopShellSelected='';
  c.state.checkedIds={};
  c.state.checkedFolder=c.state.currentFolder;
  c.state.searchResults=[];
  c.state.searchActive=-1;

  win.style.display='flex';
  win.style.position='fixed';
  win.style.left=(40+((seq-2)%6)*28)+'px';
  win.style.top=(36+((seq-2)%6)*24)+'px';
  win.style.width='calc(100vw - 120px)';
  ...
  jplopsoft_EXPLORER_INSTANCES[c.instanceId]=c;

  hwnd=jplopsoft_AdoptWindow({
    className:'ExFS.Explorer',
    windowId:c.windowId,
    titlebarId:c.titlebarId,
    titleId:c.titleId,
    clientIds:[c.idMap['jplopsoft_mainContent']],
    appId:c.appId,
    icon:'explorer',
    displayMode:'flex',
    style:jplopsoft_WS_OVERLAPPEDWINDOW,
    exStyle:jplopsoft_WS_EX_APPWINDOW,
    minButtonId:c.minButtonId,
    maxButtonId:c.maxButtonId,
    closeButtonId:c.closeButtonId,
    param:{
      ntProcessKey:'proc:explorer',
      ntImageName:'explorer.exe',
      ntDescription:'ExOS Explorer / ExFS Shell'
    },
    onMinimize:function(){jplopsoft_explorerMinimizeInstance(c.instanceId);},
    onMaximize:function(){
      jplopsoft_explorerSelectInstance(c.instanceId);
      jplopsoft_wmSetMaximized(c.windowId,true);
    },
    onRestore:function(){
      jplopsoft_explorerSelectInstance(c.instanceId);
      jplopsoft_wmSetMaximized(c.windowId,false);
      c.visible=true;
    },
    onClose:function(){jplopsoft_explorerCloseInstance(c.instanceId);}
  });

  c.hwnd=hwnd;
  rec=jplopsoft_user32GetRecord(hwnd);
  if(rec)rec.dynamic=true;

  jplopsoft_explorerBindInstance(c);
  jplopsoft_taskbarEnsureApp(c.appId,'explorer','檔案總管 #'+String(seq));

  jplopsoft_explorerSelectInstance(c.instanceId);
  jplopsoft_explorerLoadState(c);

  jplopsoft_renderAll();
  jplopsoft_explorerCaptureState(c);

  if(hwnd)jplopsoft_user32Activate(jplopsoft_user32GetRecord(hwnd));
  c.visible=true;

  return c;
}

function jplopsoft_explorerPrimaryHwnd(c){
  if(!c)return 0;
  if(c.hwnd&&jplopsoft_user32GetRecord(c.hwnd))return c.hwnd;
  c.hwnd=jplopsoft_user32GetHwndByElementId(c.windowId)||0;
  return c.hwnd;
}

function jplopsoft_explorerOpenPrimary(folderId){
  var c=jplopsoft_explorerRegisterPrimary(),w,hwnd;
  if(!c)return null;
  ...
  if(w){
    ...
    }catch(ignoreExplorerPrimaryStyle){
      w.style.display='flex';
      w.style.visibility='visible';
      w.style.position='fixed';
    }
    if(!w.style.left)w.style.left='40px';
    if(!w.style.top)w.style.top='36px';
    if(!w.style.width)w.style.width='calc(100vw - 80px)';
    if(!w.style.height)w.style.height='calc(100vh - 116px)';
  }
  ...
  jplopsoft_renderAll();
  jplopsoft_explorerCaptureState(c);
  jplopsoft_taskbarEnsureApp(c.appId,'explorer','檔案總管');

  hwnd=jplopsoft_explorerPrimaryHwnd(c);
  if(hwnd)jplopsoft_user32Activate(jplopsoft_user32GetRecord(hwnd));
  else jplopsoft_wmActivate(c.windowId,c.appId);

  return c;
}

function jplopsoft_explorerOpenNew(folderId){
  var primary=jplopsoft_explorerRegisterPrimary();
  if(!primary)return null;

  if(!primary.opened){
    return jplopsoft_explorerOpenPrimary(folderId);
  }

  return jplopsoft_explorerCreateClone(folderId);
}

function jplopsoft_explorerActivateInstance(id,bringToFront){
  var c=jplopsoft_explorerInstance(id),rec;
  if(!c)return false;
  ...
  if(bringToFront!==false){
    rec=c.hwnd?jplopsoft_user32GetRecord(c.hwnd):null;
    if(!rec&&c.primary){
      c.hwnd=jplopsoft_user32GetHwndByElementId(c.windowId)||0;
      rec=c.hwnd?jplopsoft_user32GetRecord(c.hwnd):null;
    }
    if(rec)jplopsoft_user32Activate(rec);
    else jplopsoft_wmActivate(c.windowId,c.appId);
  }
...
function jplopsoft_explorerMinimizeInstance(id){
  var c=jplopsoft_explorerInstance(id),w;
  if(!c)return false;

  if(jplopsoft_EXPLORER_ACTIVE_ID===c.instanceId){
    jplopsoft_explorerCaptureState(c);
  }
...
function jplopsoft_explorerRestoreInstance(id){
  var c=jplopsoft_explorerInstance(id),w,rec;
  if(!c)return false;
...
function jplopsoft_explorerCloseInstance(id){
  var c=jplopsoft_explorerInstance(id),w,oldActive,k,next=null;
  if(!c)return false;

  oldActive=(jplopsoft_EXPLORER_ACTIVE_ID===c.instanceId);

  if(c.searchTimer){
    try{window.clearTimeout(c.searchTimer);}catch(ignoreExplorerSearchClose){}
    c.searchTimer=0;
  }
  ...
  if(c.primary){
    ...
    c.visible=false;
    c.opened=false;
    jplopsoft_taskbarRemoveApp(c.appId);
  }else{
    if(c.hwnd&&jplopsoft_user32GetRecord(c.hwnd)){
      jplopsoft_DestroyWindow(c.hwnd);
    }else{
      w=document.getElementById(c.windowId);
      if(w&&w.parentNode)w.parentNode.removeChild(w);
      jplopsoft_taskbarRemoveApp(c.appId);
    }
    delete jplopsoft_EXPLORER_INSTANCES[c.instanceId];
  }

  if(oldActive){
    for(k in jplopsoft_EXPLORER_INSTANCES){
      if(!jplopsoft_EXPLORER_INSTANCES.hasOwnProperty(k))continue;
      if(jplopsoft_EXPLORER_INSTANCES[k].visible){
        next=jplopsoft_EXPLORER_INSTANCES[k];
        break;
      }
    }
    if(!next)next=jplopsoft_explorerInstance('1');
    if(next){
      jplopsoft_EXPLORER_ACTIVE_ID=next.instanceId;
      jplopsoft_explorerLoadState(next);
    }
  }
...
function jplopsoft_explorerCloseAllInstances(keepPrimary){
  var ids=[],k,c,i;
  for(k in jplopsoft_EXPLORER_INSTANCES){
    if(!jplopsoft_EXPLORER_INSTANCES.hasOwnProperty(k))continue;
    ids.push(k);
  ...
  for(i=0;i<ids.length;i++){
    c=jplopsoft_explorerInstance(ids[i]);
    if(!c)continue;
    if(c.primary&&keepPrimary){
      jplopsoft_explorerMinimizeInstance(c.instanceId);
      c.opened=false;
      jplopsoft_taskbarRemoveApp(c.appId);
    }else{
      jplopsoft_explorerCloseInstance(c.instanceId);
    }
  }
}

function jplopsoft_explorerMinimizeAllInstances(){
  var k,c;
  for(k in jplopsoft_EXPLORER_INSTANCES){
    if(!jplopsoft_EXPLORER_INSTANCES.hasOwnProperty(k))continue;
    c=jplopsoft_EXPLORER_INSTANCES[k];
    if(c&&c.visible)jplopsoft_explorerMinimizeInstance(c.instanceId);
  }
}

function jplopsoft_explorerRenderAllInstances(){
  var active=String(jplopsoft_EXPLORER_ACTIVE_ID||''),ids=[],k,i,c;
  for(k in jplopsoft_EXPLORER_INSTANCES){
    if(jplopsoft_EXPLORER_INSTANCES.hasOwnProperty(k))ids.push(k);
  }
  ...
  for(i=0;i<ids.length;i++){
    ...
    jplopsoft_explorerRunInInstance(c.instanceId,function(){
      jplopsoft_renderAll();
      var q=jplopsoft_el('jplopsoft_fileSearchInput');
      if(q&&jplopsoft_trim(q.value))jplopsoft_renderFileSearch(q.value);
    });
  }

  if(active&&jplopsoft_explorerInstance(active)){
    jplopsoft_EXPLORER_ACTIVE_ID=active;
    jplopsoft_explorerLoadState(jplopsoft_explorerInstance(active));
  }
}
...
function jplopsoft_explorerBeginOpen(){
  ...
  jplopsoft_EXPLORER_LIFECYCLE.guardUntil=jplopsoft_EXPLORER_LIFECYCLE.lastOpenAt+250;
...
function jplopsoft_explorerPromoteForeground(w){
  var c=w?jplopsoft_explorerContextByWindowId(w.id):null;
  if(!c)return false;
  return jplopsoft_explorerActivateInstance(c.instanceId,true);
}
...
function jplopsoft_explorerHealthCheck(){
  var c=jplopsoft_explorerCurrentContext(),w;
  if(!c||!c.visible)return false;
  w=document.getElementById(c.windowId);
  if(!w)return false;
  if(!jplopsoft_explorerComputedVisible(w)){
    w.style.display='flex';
    w.style.visibility='visible';
    w.style.pointerEvents='auto';
  }
...
function jplopsoft_wmActivate(windowId,appId){
  ...
  if(String(appId||'').indexOf('explorer')===0){
    jplopsoft_explorerSelectByAppId(appId);
  }
...
function jplopsoft_explorerPortalToRoot(w){
  ...
  if(w.parentNode!==host){
    try{host.appendChild(w);}catch(ignoreExplorerPortal){return false;}
  }
  jplopsoft_wmClassAdd(w,'jplopsoft_explorer-root-portal');
  try{w.setAttribute('data-exos-window-host','BODY');}catch(ignoreExplorerHostAttr){}
  return true;
}
...
function jplopsoft_wmMinimize(windowId,appId,explicitAction){
  ...
  if(String(appId||'').indexOf('explorer')===0){
    c=jplopsoft_explorerContextByAppId(appId);
    if(c)return jplopsoft_explorerMinimizeInstance(c.instanceId);
  }
...
function jplopsoft_wmRestoreExplorerStable(){
  ...
  if(!c)return false;
  return jplopsoft_explorerRestoreInstance(c.instanceId);
}
...
function jplopsoft_wmCloseExplorer(explicitAction){
  var c=jplopsoft_explorerCurrentContext()||jplopsoft_explorerInstance('1');
  if(!c)return false;
  return jplopsoft_explorerCloseInstance(c.instanceId);
}
...
function jplopsoft_wmShowDesktop(){
  ...
  if(jplopsoft_el(jplopsoft_taskbarAppId('versions')))jplopsoft_versionWindowMinimize();
  ...
  (function(){
    ...
    for(k in jplopsoft_MULTI_EDITORS){
      if(jplopsoft_MULTI_EDITORS.hasOwnProperty(k)){
        r=jplopsoft_MULTI_EDITORS[k];
        jplopsoft_multiEditorMinimize(r);
      }
    }
  })();
...
function jplopsoft_wmAfterUnlock(){
  ...
  if(primary){
    ...
    primary.state.selectedId=0;
    primary.state.checkedIds={};
    jplopsoft_explorerLoadState(primary);
  }
...
function jplopsoft_wmPrepareLock(){
  ...
  if(jplopsoft_el('jplopsoft_trashBackdrop')){
    jplopsoft_el('jplopsoft_trashBackdrop').style.display='none';
  }
...
function jplopsoft_taskbarAppClick(appId){
  ...
  if(appId==='explorer'||String(appId||'').indexOf('explorer_')===0){
    (function(){
      var c=jplopsoft_explorerContextByAppId(appId);
      if(!c)return;
      if(active&&c.visible){
        ...
        jplopsoft_explorerRestoreInstance(c.instanceId);
      }
    })();
    return;
  }
  ...
  if(appId==='versions'){
    if(
      active&&
      jplopsoft_wmIsDisplayed('jplopsoft_versionBackdrop')
    ){
      jplopsoft_versionWindowMinimize();
    }else{
      jplopsoft_versionWindowRestore();
    }
    return;
  }
  if(appId==='security'){
...
function jplopsoft_ntKernelProcessKeyForWindow(rec){
  ...
  if(app==='explorer'||app.indexOf('explorer_')===0||app==='trash'||app==='properties'||app==='document'||app==='versions'){
...
function jplopsoft_ntKernelImageForWindow(rec){
  ...
  if(app==='explorer'||app.indexOf('explorer_')===0||app==='trash'||app==='properties'||app==='document'||app==='versions')return'explorer.exe';
  ...
  if(app.indexOf('xsh_')===0)return'xshhost.exe';
...
function jplopsoft_ntKernelDescriptionForWindow(rec){
  ...
  if(app==='explorer'||app.indexOf('explorer_')===0||app==='trash'||app==='properties'||app==='document'||app==='versions')return'ExOS Explorer / ExFS Shell';
  ...
  if(app.indexOf('xsh_')===0)return'XSH Sandbox Host';
...
async function jplopsoft_xshCreateCNode(ctx,path,type){
  ...
  if(!jplopsoft_isWritableProfileFolder(parent.parentId))throw jplopsoft_xshError(jplopsoft_STATUS_ACCESS_DENIED,'XSH can create objects only inside the current user profile.');
  if(type==='file'){
...
function jplopsoft_xshCreateAppWindow(ctx,spec){
  ...
  h=jplopsoft_CreateWindowEx(
    jplopsoft_WS_EX_APPWINDOW,'ExOS.Window',String(s.title||ctx.name),jplopsoft_WS_OVERLAPPEDWINDOW|jplopsoft_WS_VISIBLE,
...
function jplopsoft_xshTopologySnapshot(mode){
  ...
  out.push({id:'mft',parentId:'phys_root',label:'_MFT.x6f',kind:'metadata',detail:'Master File Table / nodes / versions'});
...
async function jplopsoft_xshPromptBox(ctx,message,title,defaultValue){
  ...
  result=await jplopsoft_exosPrompt(
    caption+'\n\n'+text,
...
async function jplopsoft_xshConfirmBox(ctx,message,title){
  return !!await jplopsoft_exosConfirm(
...
async function jplopsoft_xshDispatch(ctx,api,method,args){
  ...
  if(api==='kernel32'){
    ...
    if(method==='CloseHandle'){if(jplopsoft_xshCloseHandle(ctx,args[0]))return true;if(typeof jplopsoft_advapi32CloseHandle==='function'&&jplopsoft_advapi32CloseHandle(ctx,args[0]))return true;throw jplopsoft_xshError(jplopsoft_STATUS_INVALID_HANDLE,'Invalid handle.');}
...
function jplopsoft_launchSystemXshApp(appId,args){jplopsoft_runBuiltinXsh(appId,args||[]).catch(function(e){jplopsoft_exosMessage('XSH app 啟動失敗：'+String(e&&e.message?e.message:e));});}
...
function jplopsoft_openCalculator(){if(!state.samAuthenticated||!state.vaultKey){jplopsoft_exosMessage('請先登入 ExOS。');return;}jplopsoft_launchSystemXshApp('calc',[]);}
...
async function jplopsoft_taskmgrEndTask(pid){
  ...
  title=p.imageName;
  if(!await jplopsoft_exosConfirm('要將 WM_CLOSE 傳送給「'+title+'」的所有頂層視窗嗎？'))return;
...
async function jplopsoft_taskmgrTerminateProcess(pid){
...
function jplopsoft_regeditWriteValue(name,type,data){
  ...
  jplopsoft_api('reg_open_key','POST',{hive:hive,key:key,desired_access:'KEY_WRITE'},true,function(err,out){
    var h;
    if(err){jplopsoft_exosMessage(err.message);return;}
    h=out.handle;
    jplopsoft_api('reg_set_value','POST',{handle:h,name:name,type:type,data:data},true,function(err2){
      ...
      if(err2){jplopsoft_exosMessage(err2.message);return;}
      jplopsoft_regeditLoad();
    });
  });
}

async function jplopsoft_regeditEditValue(){
  ...
  data=await jplopsoft_exosPrompt('編輯 '+(name===''?'(預設)':name)+'\\n類型：'+type,String(typeof v.data==='undefined'?'':v.data));
  if(data===null)return;
  if(type==='REG_DWORD')data=parseInt(data,10)||0;
  jplopsoft_regeditWriteValue(name,type,data);
}

async function jplopsoft_regeditNewValue(){
  var name=await jplopsoft_exosPrompt('新增值名稱\\n留空代表 (預設)','');
  if(name===null)return;
  var type=await jplopsoft_exosPrompt('值類型：REG_SZ / REG_DWORD / REG_BINARY','REG_SZ');
  if(type===null)return;
  type=String(type||'REG_SZ').toUpperCase();
  if(type!=='REG_SZ'&&type!=='REG_DWORD'&&type!=='REG_BINARY'){jplopsoft_exosMessage('不支援的 Registry 類型。');return;}
  var data=await jplopsoft_exosPrompt('值資料','');
  if(data===null)return;
  if(type==='REG_DWORD')data=parseInt(data,10)||0;
  jplopsoft_regeditWriteValue(String(name),type,data);
}

async function jplopsoft_regeditNewKey(){
  var name=await jplopsoft_exosPrompt('新增子機碼名稱','New Key');
...
async function jplopsoft_regeditDeleteValue(){
  var name=jplopsoft_REGEDIT.selectedValue;
  if(name===null)return;
  if(!await jplopsoft_exosConfirm('要刪除 Registry 值「'+(name===''?'(預設)':name)+'」嗎？'))return;
...
async function jplopsoft_regeditDeleteKey(){
  var key=jplopsoft_REGEDIT.key,parent;
  if(!key){jplopsoft_exosMessage('不能刪除 Registry Hive 根節點。');return;}
  if(!await jplopsoft_exosConfirm('要刪除機碼與全部子機碼嗎？\\n\\n'+jplopsoft_regeditFullPath(jplopsoft_REGEDIT.hive,key)))return;
...
function jplopsoft_taskbarCloseAppWindow(appId){
  ...
  if(appId==='explorer'||appId.indexOf('explorer_')===0){
    var ec=jplopsoft_explorerContextByAppId(appId);
    if(ec){jplopsoft_explorerCloseInstance(ec.instanceId);return true;}
  }
  ...
  if(appId==='versions'){jplopsoft_closeVersions();return true;}
  if(appId==='security'){
...
function jplopsoft_RegisterExFSNativeWindows(){
  ...
  if(!jplopsoft_user32GetHwndByElementId('jplopsoft_explorerWindow')){
    hwnd=jplopsoft_AdoptWindow({
      ...
      maxButtonId:'jplopsoft_explorerMaxBtn',
      closeButtonId:'jplopsoft_explorerCloseBtn',
      onMinimize:function(){jplopsoft_explorerMinimizeInstance('1');},
      onMaximize:function(){jplopsoft_explorerSelectInstance('1');jplopsoft_wmSetMaximized('jplopsoft_explorerWindow',true);},
      onRestore:function(){jplopsoft_explorerSelectInstance('1');jplopsoft_wmSetMaximized('jplopsoft_explorerWindow',false);jplopsoft_explorerRestoreInstance('1');},
      onClose:function(){jplopsoft_explorerCloseInstance('1');}
    });
  }

  (function(){
    var c=jplopsoft_explorerRegisterPrimary();
    if(c){
      c.hwnd=jplopsoft_user32GetHwndByElementId(c.windowId)||0;
      jplopsoft_explorerBindInstance(c);
    }
  ...
  if(!jplopsoft_user32GetHwndByElementId('jplopsoft_versionWindow')){
    hwnd=jplopsoft_AdoptWindow({
      className:'ExFS.VersionHistory',
      overlay:true,
      backdropId:'jplopsoft_versionBackdrop',
      windowId:'jplopsoft_versionWindow',
      titlebarId:'jplopsoft_versionTitlebar',
      titleId:'jplopsoft_versionTitle',
      clientIds:['jplopsoft_versionRows'],
      appId:'versions',
      icon:'history',
      style:jplopsoft_WS_OVERLAPPEDWINDOW,
      exStyle:jplopsoft_WS_EX_APPWINDOW,
      minButtonId:'jplopsoft_versionMinBtn',
      maxButtonId:'jplopsoft_versionMaxBtn',
      closeButtonId:'jplopsoft_versionCloseTop',
      onMinimize:function(){jplopsoft_versionWindowMinimize();},
      onMaximize:function(){
        jplopsoft_versionWindowSetMaximized(true);
        jplopsoft_wmActivateOverlay(
          'jplopsoft_versionBackdrop',
          'jplopsoft_versionWindow',
          'versions'
      ...
      onClose:function(){jplopsoft_closeVersions();}
    });
  }
...
function jplopsoft_bindWindowManager(){
  var explorer=jplopsoft_el('jplopsoft_explorerWindow'),
      ...
      versionWin=jplopsoft_el('jplopsoft_versionWindow'),
  ...
  jplopsoft_dwmNormalizeRootStacking();
  jplopsoft_explorerRegisterPrimary();
  if(explorer)jplopsoft_explorerPortalToRoot(explorer);
  jplopsoft_applyDesktopWallpaper();

  if(explorer){
    explorer.onmousedown=function(){jplopsoft_explorerActivateInstance('1',true);};
  ...
  if(versionWin){
    versionWin.onmousedown=function(){
      jplopsoft_wmActivateOverlay(
        'jplopsoft_versionBackdrop',
        'jplopsoft_versionWindow',
        'versions'
    ...
    jplopsoft_wmMakeDraggable(
      'jplopsoft_versionWindow',
      'jplopsoft_versionTitlebar'
    );
  }
  if(security){
  ...
  if(jplopsoft_el('jplopsoft_explorerMinBtn'))jplopsoft_el('jplopsoft_explorerMinBtn').onclick=function(){jplopsoft_explorerMinimizeInstance('1');};
  if(jplopsoft_el('jplopsoft_explorerMaxBtn'))jplopsoft_el('jplopsoft_explorerMaxBtn').onclick=function(){jplopsoft_explorerActivateInstance('1',true);jplopsoft_wmToggleMax('jplopsoft_explorerWindow');};
  if(jplopsoft_el('jplopsoft_explorerCloseBtn'))jplopsoft_el('jplopsoft_explorerCloseBtn').onclick=function(){jplopsoft_explorerCloseInstance('1');};
  ...
  if(jplopsoft_el('jplopsoft_versionMinBtn'))jplopsoft_el('jplopsoft_versionMinBtn').onclick=jplopsoft_versionWindowMinimize;
  if(jplopsoft_el('jplopsoft_versionMaxBtn'))jplopsoft_el('jplopsoft_versionMaxBtn').onclick=jplopsoft_versionWindowToggleMax;
  if(jplopsoft_el('jplopsoft_versionCloseTop'))jplopsoft_el('jplopsoft_versionCloseTop').onclick=jplopsoft_closeVersions;
  ...
  if(desktopComputer){
    desktopComputer.ondblclick=function(e){
      e=e||window.event;
      if(e&&e.preventDefault)e.preventDefault();
  ...
  /*
   * RegisterExFSNativeWindows() adopts the legacy/static windows and may add
   * their HWND metadata after the first pass.  Rebind once more afterwards:
   * USER32 is the single final owner of every top-right NC button.
   */
...
function jplopsoft_documentChromeInfo(fmt){
  fmt=String(fmt||'').toLowerCase();
  if(fmt==='csv')return{icon:'csv',title:'CSV'};
  if(fmt==='xsh')return{icon:'xsh',title:'XSH'};
  if(fmt==='txt')return{icon:'txt',title:'TXT'};
  if(fmt==='image')return{icon:'file',title:'圖片'};
  if(fmt==='media')return{icon:'file',title:'媒體'};
  return{icon:'html',title:'HTML'};
}

function jplopsoft_documentChromeApply(fmt){
  var info=jplopsoft_documentChromeInfo(fmt),
      icon=jplopsoft_el('jplopsoft_documentTitleIcon');

  if(icon){
    icon.setAttribute('data-exfs-svg',info.icon);
    icon.removeAttribute('data-exfs-svg-applied');
    icon.style.backgroundImage='';
    jplopsoft_applySvgIcons(icon.parentNode||document);
  }
}
...
function jplopsoft_taskbarDocumentFormatInfo(fmt){
  ...
  if(fmt==='xsh')return{icon:'xsh',app:'XSH 程式'};
...
function jplopsoft_taskbarSetDocumentApp(id,name,fmt){
  ...
  jplopsoft_documentChromeApply(fmt);
...
function jplopsoft_calendarToggle(){jplopsoft_calendarHide();if(!state.samAuthenticated||!state.vaultKey)return;jplopsoft_launchSystemXshApp('calendar',[]);}
...
/* os52: Control Panel, Recycle Bin and 3D Topology remain XSH system apps; GDI32 is now available to all XSH GUI processes. */
function jplopsoft_openExconfig(){
  if(!state.samAuthenticated||!state.vaultKey){jplopsoft_exosMessage('請先登入 ExOS。');return false;}
  jplopsoft_launchSystemXshApp('control',[]);return true;
}
function jplopsoft_closeExconfig(){jplopsoft_xshTerminateBuiltin('control');return true;}
function jplopsoft_openTrash(){
  if(!state.samAuthenticated||!state.vaultKey){jplopsoft_exosMessage('請先登入 ExOS。');return false;}
  jplopsoft_launchSystemXshApp('trash',[]);return true;
}
function jplopsoft_closeTrash(){jplopsoft_xshTerminateBuiltin('trash');return true;}
function jplopsoft_openVolume3D(){
  if(!state.samAuthenticated||!state.vaultKey){jplopsoft_exosMessage('請先登入 ExOS。');return false;}
  jplopsoft_launchSystemXshApp('volume3d',[]);return true;
}
