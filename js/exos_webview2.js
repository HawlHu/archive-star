...
var API={version:'6.4.0-dev-os91-hotfix10',model:'EXOS_WEBVIEW2_V1',ready:true};
...
async function dispatch(ctx,method,args){args=args||[];method=String(method||'');var s=state(ctx),h,v,env,hwnd,client,frame,obs;
 ...
 if(method==='CreateCoreWebView2Controller'){var opt=args[2]||{},parentId=String(opt.parentId||'');env=s.envs[String(parseInt(args[0],10)||0)];if(!env)throw exerr(st('INVALID_HANDLE',0xC0000008),'Invalid WebView2 environment.');hwnd=parseInt(args[1],10)||0;if(!ctx.windows||!ctx.windows[String(hwnd)])throw exerr(st('ACCESS_DENIED',0xC0000022),'HWND is not owned by this XSH process.');client=null;if(parentId&&typeof global.jplopsoft_xshControl==='function')client=global.jplopsoft_xshControl(ctx,parentId);if(!client)client=typeof global.jplopsoft_GetClientElement==='function'?global.jplopsoft_GetClientElement(hwnd):null;if(!client)throw exerr(st('INVALID_HANDLE',0xC0000008),parentId?'WebView2 parent control unavailable: '+parentId:'Window client area unavailable.');frame=document.createElement('iframe');frame.className='jplopsoft-webview2';frame.setAttribute('sandbox','allow-scripts');frame.setAttribute('referrerpolicy','no-referrer');frame.setAttribute('allow','');frame.style.cssText='position:absolute;inset:0;width:100%;height:100%;border:0;background:#fff;z-index:1;';try{if(getComputedStyle(client).position==='static')client.style.position='relative';}catch(ignore){}client.appendChild(frame);h=s.next++;v={handle:h,env:env.handle,hwnd:hwnd,parentId:parentId,frame:frame,source:'about:blank',kind:'url',history:[],index:-1,messageHandler:null,observer:null};s.views[String(h)]=v;frame.onload=function(){notify(ctx,v,'navigationcompleted',{source:v.kind==='url'?v.source:'about:srcdoc',success:true});};v.messageHandler=function(e){try{if(e.source===frame.contentWindow)notify(ctx,v,'webmessage',{data:e.data,origin:String(e.origin||'null')});}catch(ignore){}};window.addEventListener('message',v.messageHandler,false);if(typeof ResizeObserver==='function'){obs=new ResizeObserver(function(){notify(ctx,v,'boundschanged',{});});try{obs.observe(client);}catch(ignoreObs){}v.observer=obs;}return h;}
